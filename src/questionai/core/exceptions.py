...
class QuestionAIError(Exception):
    """Base exception for all QuestionAI errors."""
    pass


# ── Ingestion ────────────────────────────────────────────────────────────────

class FileDiscoveryError(QuestionAIError):
    """Raised when the papers directory cannot be scanned."""
    pass


class FilenameParseError(QuestionAIError):
...
class PDFLoadError(QuestionAIError):
...
class PDFClassificationError(QuestionAIError):
    """Raised when PDF type classification fails."""
    pass


class PageRenderError(QuestionAIError):
...
class OCRError(QuestionAIError):
...
class OCREngineNotAvailable(QuestionAIError):
...
class LayoutAnalysisError(QuestionAIError):
...
class ExtractionError(QuestionAIError):
...
class QuestionDetectionError(QuestionAIError):
...
class ValidationError(QuestionAIError):
...
class OutputWriteError(QuestionAIError):
...
class ConfigError(QuestionAIError):
