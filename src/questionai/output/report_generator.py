...
from questionai.core.models import ExtractionReport, ProcessingStatus
...
def print_batch_summary(all_reports: List[ExtractionReport], total_papers: Optional[int] = None) -> None:
    ...
    console = Console()
    table = Table(title=f"QuestionAI Stage A Extraction Summary — [{parsed_count}/{total_count} Papers Parsed ({pct:.1f}%)]")
